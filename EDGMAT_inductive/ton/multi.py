...
G = nx.from_pandas_edgelist(X_train,  "src_ip", "dst_ip", ['h','label'],create_using=nx.MultiDiGraph())
...
G_test = nx.from_pandas_edgelist(X_test,  "src_ip", "dst_ip", ['h','label'],create_using=nx.MultiDiGraph())
...
print(cm)
target_names =[ str(i) for i in list(np.unique(actual))]
